...
로컬 집계 파일의 검증과 전송은 비공개 `H1june/morimens-data-extractor` 저장소가 담당한다. 추출기는 집계 파일을 원자적으로 저장한 뒤 전송 명령을 실행해야 한다. 파일 저장만으로는 운영 API가 갱신되지 않는다.
...
자정 수집, 원본 체크포인트, 집계와 게시 도구는 비공개 `H1june/morimens-data-extractor` 저장소가 소유한다. 이 공개 저장소는 Worker의 수신·조회 API와 브라우저 표시, 공개 산출물 검증만 소유한다.

비공개 수집기가 게시에 실패하면 Worker의 마지막 정상 집계를 유지한다. 원본 식별자와 로컬 체크포인트는 이 저장소와 Pages 배포 아티팩트에 포함하지 않는다. 실제 예약 작업 설치 여부와 수집기 지원 버전은 비공개 저장소 README를 기준으로 확인한다.
