...
- 사이트 툴팁에는 해당 키워드·상태가 **직접 수행하는 효과만** 짧고 완결되게 적는다.
- 다른 상태와의 관계, 발동 원인, 예외, 전략적 해설은 그 다른 상태 또는 관련 규칙의 설명에 둔다. 해당 항목의 효과가 아닌 부연은 툴팁에 넣지 않는다.
- 설명은 인게임 원문과 확인된 상태·스킬 데이터에 근거하며, 추정이나 해석을 추가하지 않는다.
- 직접 효과의 발동 조건·지속 시간·횟수 제한은 사족이 아니며 생략하지 않는다. 같은 이름이라도 실제 부여되는 State/Skill ID가 다르면 별도 검증한다.
- 융재금구 설명 갱신은 [패치 데이터 갱신 절차](dzone-content-pipeline.md)를 따른다. 몬스터 이름·한국어 단어만 맞춰 툴팁을 연결하지 않는다.
