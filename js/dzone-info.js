(() => {
    ...
    function dynamicMarkup(description, options = {}) {
        const cleaned = politeText(description || '상세 설명이 없습니다.')
            ...
            .replace(/\[Damage:Arg\d+\]\s*(?:pt|포인트)?/g, '현재 상태에 따라 달라지는 수치')
            .replace(/\[AttackTimes:Arg\d+\]\s*회/g, '조건에 따라 정해진 횟수만큼')
            .replace(/재생력\s*\[Arg\d+\]\s*스택을 획득/g, '재생력을 획득')
            .replace(/\[(?:[A-Za-z]+:)?(?:Arg|StateArg|DescArg)\d+\](?:pt|포인트)?/g, '')
            .replace(/\[Layer\]/g, '')
            .replace(/\(\s*\)/g, '')
            .replace(/\s+([,.])/g, '$1')
            .replace(/\s{2,}/g, ' ')
