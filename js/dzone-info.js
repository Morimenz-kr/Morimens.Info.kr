        ...
        const request = (async () => {
            for (const url of urls) {
                try {
                    ...
                    const legacyDifficulties = ['normal', 'hard', 'nightmare', 'madness'];
                    const highDifficulties = ['nightmare', 'madness'];
                    const gradeDifficulties = ['1', '2', '3', '4', '5', '6', '7'];
                    const rawDifficulties = stages?.map(stage => String(stage?.difficulty ?? '')) || [];
                    const difficulties = stages?.length === 10 && rawDifficulties.every(value => highDifficulties.includes(value))
                        ? highDifficulties
                        : stages?.length === 20 && rawDifficulties.every(value => legacyDifficulties.includes(value))
                            ? legacyDifficulties
                            : stages?.length === 35 && rawDifficulties.every(value => gradeDifficulties.includes(value))
                                ? gradeDifficulties
                                : null;
                    if (!payload?.data || !difficulties || !Number.isInteger(payload.fetchedAt)) continue;
