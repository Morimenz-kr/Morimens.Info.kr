...
// [4] 태그 및 메타데이터 정의
const ALL_KEY_TAGS = [ "산출력", "산출력 획득", "은열쇠 에너지", "은열쇠 게이지", "방어막 획득", "체력 회복", "힘", "힘 증가", "피해 증폭", "강효", "치명타 확률", "치명타 확률 증가", "치명타 피해", "치명타 피해 증가", "영역 숙련", "카드 추가", "드로우", "카드 뽑기", "코스트 감소", "계산 비용", "복사본", "영감", "광기", "광기 부여", "약화", "취약", "중독", "중독 부여", "힘 훔침", "힘 감소", "반격", "소멸", "경계", "희생", "촉수", "터치 손상", "강생 의식", "스칼렛 용광로", "초월 턴", "시편", "주사위" ];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.addEventListener('touchstart', (event) => {
        if (event.target.closest('#global-tooltip, .tooltip-trigger, .character-effect-tooltip-box')) return;
        hideTooltip();
    }, { passive: true });
});
...
async function loadExternalData() {
    try {
        ...
        const [resChars, resWheels, resKeys, resRules, resTooltips] = await Promise.all([
            ...
            fetch(`data/db_tooltips.json?t=${ts}`).catch(() => null)
        ]);
        ...
        TOOLTIP_DICTIONARY = resTooltips?.ok ? await resTooltips.json() : {};
...
function getRegularCharacterIdsInPage(page, excludedTeamIdx = -1) {
    return page.teams.flatMap((team, teamIdx) => {
        if (teamIdx === excludedTeamIdx) return [];
        return team.chars.filter((id, slotIdx) => id && slotIdx !== team.supportIdx);
    });
}
...
function applySupportToCurrentTeam(charId) {
    ...
    const regularCharacterIds = getRegularCharacterIdsInPage(currentPage);
    if (hasExclusiveCharacterConflict(charId, regularCharacterIds)) {
        openSystemAlert("편성 불가", "현재 편성표에 함께 출전할 수 없는 각성체가 있습니다.");
        return false;
    }
...
function renderCharGrid() {
    ...
    const otherTeamRegularIds = getRegularCharacterIdsInPage(currentPage, currentTeamIdx);
    const usedInOtherTeamsNormal = new Set(otherTeamRegularIds);
    ...
    DB.chars.filter(c => {
        ...
        if (isSupportSelectionMode) {
            ...
            else if (hasExclusiveCharacterConflict(id, regularIdsInPage)) conflictReason = "출전 불가";
            else if (getDomainsWithSupportCandidate(team, id).size > 2) conflictReason = "영역 충돌";
        } else {
            // [일반 대원 편성 모드]
            if (usedInOtherTeamsNormal.has(id)) {
                conflictReason = "사용중";
            } else if (hasExclusiveCharacterConflict(id, otherTeamRegularIds)) {
        ...
        if (conflictReason) {
            ...
            const conflictClass = conflictReason === "영역 충돌"
                ? " domain-conflict-label"
                : (conflictReason === "출전 불가" ? " unavailable-conflict-label" : "");
            overlay.className = 'conflict-tag' + conflictClass;
...
function confirmQuickSetup() {
    ...
    const otherTeamRegularIds = getRegularCharacterIdsInPage(allPages[currentPageIdx], currentTeamIdx);
    if (tempChars.some(id => hasExclusiveCharacterConflict(id, otherTeamRegularIds))) {
        openSystemAlert("편성 불가", "다른 파티에 함께 출전할 수 없는 각성체가 포함되어 있습니다.");
        return;
    }
...
function showTooltip(item, e) {
    tooltipSourceEl = e?.currentTarget || e?.target || null;
    ...
    if (item.description) {
        const description = window.CharacterEffects
            ? window.CharacterEffects.renderRichText(item.description, TOOLTIP_DICTIONARY)
            : item.description;
        contentHtml += `<div class="tooltip-effect-desc character-effect-rich-text">${description}</div>`;
    }
    ...
    if (item.set_effect_3) {
        ...
        const setEffect3 = window.CharacterEffects
            ? window.CharacterEffects.renderRichText(item.set_effect_3, TOOLTIP_DICTIONARY)
            : item.set_effect_3;
        contentHtml += `<div class="tooltip-effect-desc character-effect-rich-text" style="margin-top: 2px;">${setEffect3}</div>`;
    }
    if (item.set_effect_6) {
        ...
        const setEffect6 = window.CharacterEffects
            ? window.CharacterEffects.renderRichText(item.set_effect_6, TOOLTIP_DICTIONARY)
            : item.set_effect_6;
        contentHtml += `<div class="tooltip-effect-desc character-effect-rich-text" style="margin-top: 2px;">${setEffect6}</div>`;
    }
    ...
    window.CharacterEffects?.setupTooltips(descCont);
...
function hideTooltip(e) {
    if (
        ...
        (
            ...
            e.relatedTarget.closest?.('.character-effect-tooltip-box')
        )
    ) return;
...
tooltipEl?.addEventListener('mouseleave', (e) => {
    if (
        e.relatedTarget &&
        (
            tooltipSourceEl?.contains?.(e.relatedTarget) ||
            e.relatedTarget.closest?.('.character-effect-tooltip-box')
        )
    ) return;
    hideTooltip();
