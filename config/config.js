...
const CONFIG = {
    ...
    VERSION: "v1.3.32",
    ASSET_REVISION: "dzone-constraints-overview-20260904-2",
...
function loadCSS(path) {
    document.write(`<link rel="stylesheet" href="${path}?v=${CONFIG.VERSION}-${CONFIG.ASSET_REVISION}">`);
}

// JS를 버전 붙여서 로드하는 함수
function loadJS(path) {
    // <\/script> 로 쓰는 이유는 자바스크립트 문자열 내에서 스크립트 태그 종료를 안전하게 처리하기 위함입니다.
    document.write(`<script src="${path}?v=${CONFIG.VERSION}-${CONFIG.ASSET_REVISION}"><\/script>`);
}
